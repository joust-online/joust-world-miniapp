...
export function ScrollWide4({ className, preserveAspectRatio }: Props) {
  ...
  return (
    ...
    >
      <defs>
        <filter id="imq-glow-edges-sw4" x="-10%" y="-10%" width="120%" height="120%" colorInterpolationFilters="sRGB">
        ...
        <clipPath id="clip-vgrain-sw4">
          <path d={VGRAIN_PATH} />
        </clipPath>
        <clipPath id="clip-grain-sw4">
          <path d={GRAIN_PATH} />
        </clipPath>
      </defs>

      <path d={VGRAIN_PATH} fill="#010101" stroke="#4c53a4" strokeMiterlimit={10} />
      <g clipPath="url(#clip-vgrain-sw4)" filter={`url(#${vgrain.filterId})`} pointerEvents="none" aria-hidden="true">
        <g fill="#010101">
          {vgrain.rects.map((r, i) => (<rect key={i} x={r.x} y={r.y} width={r.width} height={r.height} opacity={r.opacity} />))}
        </g>
      </g>

      <g filter="url(#imq-glow-edges-sw4)">
      ...
      <g clipPath="url(#clip-grain-sw4)" filter={`url(#${grain.filterId})`} pointerEvents="none" aria-hidden="true">
        <g fill="#010101">
